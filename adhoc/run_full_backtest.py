...
es_params   = {**STRATEGIES['expiry_spread']['params']}
...
def _run_strategies(data: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    print(f"  → Expiry Spread (directional 0 DTE) ...")
    es_log   = run_expiry_spread(data, inst_cfg, es_params, groww=groww)
    print(f"     {len(es_log)} trades")

    frames = [df for df in [orb_log, vwap_log, opt_log, ic_log, es_log] if not df.empty]
    if not frames:
        return pd.DataFrame()
