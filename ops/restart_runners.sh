...
caffeinate -i /opt/anaconda3/bin/python3 v3/live/runner_nifty.py > /dev/null 2>&1 &
echo "  PID $!"

echo "Starting BANKNIFTY runner (paper mode)..."
caffeinate -i /opt/anaconda3/bin/python3 v3/live/runner_banknifty.py > /dev/null 2>&1 &
echo "  PID $!"

echo "Starting News runner (alert-only)..."
# Logs to news/news_runner.log via runner.py's FileHandler.
caffeinate -i /opt/anaconda3/bin/python3 -m news.runner > /dev/null 2>&1 &
echo "  PID $!"

echo ""
echo "Runners started. Tailing logs (Ctrl+C to detach — runners keep going)"
sleep 2
tail -f v3/live/runner.log v3/live/runner_banknifty.log news/news_runner.log
