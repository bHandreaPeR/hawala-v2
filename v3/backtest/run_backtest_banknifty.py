"""
v3/backtest/run_backtest_banknifty.py
==================================
Full 6-signal V3 backtest for BANKNIFTY — simulates real option trades (buy ATM CE/PE).
...
Data sources (all lag-1 / no lookahead):
  ...
  - Per-strike option 1m LTP:  v3/cache/option_oi_1m_BANKNIFTY.pkl  (needs close column)
  - Bhavcopy EOD:              v3/cache/bhavcopy_BN_all.pkl  (WARNING if missing)
...
DTE: nearest TUESDAY expiry (BankNifty monthly = last Tuesday)

Usage: python v3/backtest/run_backtest_banknifty.py
"""
...
log = logging.getLogger('backtest_banknifty')
...
# FII/DII classifiers — three variants run in parallel for comparison
# clf_nifty   : Nifty-trained thresholds (baseline, wrong instrument but useful reference)
# clf_bn      : BankNifty-only calibrated thresholds + BN-specific ATM/OTM constants
# clf_combined: Combined Nifty+BankNifty thresholds, BN runtime constants
#
# All three are optional; backtest degrades gracefully if thresholds missing.
# Trade signal uses clf_bn as primary. All three results logged to CSV.

_clf         = None    # Nifty thresholds (reference)
_clf_bn      = None    # BankNifty-only thresholds (primary for BN)
_clf_combined= None    # Combined thresholds, BN runtime
_FII_CLF_AVAILABLE = False

try:
    from v3.signals.fii_dii_classifier import FIIDIIClassifier as _NiftyCLF, OISnapshot
    _clf = _NiftyCLF()
    _FII_CLF_AVAILABLE = True
    print("FII/DII classifier (NIFTY thresholds) loaded")
except Exception as _e:
    print(f"WARNING: Nifty classifier unavailable: {_e}")

try:
    from v3.signals.fii_dii_classifier_BANKNIFTY import FIIDIIClassifier as _BNCLF
    _clf_bn = _BNCLF()
    _FII_CLF_AVAILABLE = True
    print("FII/DII classifier (BANKNIFTY thresholds) loaded")
except Exception as _e:
    print(f"WARNING: BankNifty classifier unavailable: {_e}")

try:
    from v3.signals.fii_dii_classifier_COMBINED import (
        FIIDIIClassifier as _CombinedCLF, OISnapshot,
    )
    _clf_combined = _CombinedCLF(instrument='BANKNIFTY')
    _FII_CLF_AVAILABLE = True
    print("FII/DII classifier (COMBINED thresholds) loaded")
except Exception as _e:
    print(f"WARNING: Combined classifier unavailable: {_e}")
...
if not candle_file.exists():
    raise FileNotFoundError(
        ...
        f"Run v3/data/fetch_1m_BANKNIFTY.py first."
    )
with open(candle_file, 'rb') as f:
    candles = pickle.load(f)
trade_dates = sorted(candles['date'].unique())
print(f"BANKNIFTY candles: {len(candles)} rows, {len(trade_dates)} trading days")

# ── Pre-compute 20-day realized vol map ───────────────────────────────────────
# vol_20d = rolling std of daily % returns over 20 trading days (min 10).
# Used by F_VOL gate: skip days where BankNifty options are too thin to profit.
_daily_close  = candles.groupby('date')['close'].last().sort_index()
_daily_ret    = _daily_close.pct_change() * 100          # daily % return
_vol_series   = _daily_ret.rolling(20, min_periods=10).std()
vol_20d_map   = _vol_series.to_dict()                    # {date: vol_20d_pct}
print(f"Vol map built: {len(vol_20d_map)} dates, "
      f"current vol={list(_vol_series.dropna())[-1]:.3f}%")

# Bhavcopy for NIFTY — if missing, warn loudly and degrade gracefully
bhav_file = ROOT / 'v3/cache/bhavcopy_BN_all.pkl'
...
if bhav_file.exists():
    ...
    print(f"BANKNIFTY bhavcopy: {len(bhav_dates)} dates")
    ...
    print(
        "\nWARNING: bhavcopy_BN_all.pkl NOT FOUND at %s" % bhav_file
    )
    print("WARNING: Falling back to OI cache for PCR + walls (EOD snapshot, lag-1).")
    print("         Run python v3/data/fetch_bhavcopy_nifty.py (NSE bhavcopy covers BankNifty too) for full NSE history.\n")
...
if opt_oi_file.exists():
    ...
    print(f"BANKNIFTY option OI 1m cache: {len(opt_oi_cache)} days")
else:
    opt_oi_cache = {}
    print("WARNING: option_oi_1m_BANKNIFTY.pkl not found — OI velocity will be 0")
    print("         Run: python v3/data/fetch_option_oi_BANKNIFTY.py")
...
# Spot proxy from BankNifty futures daily close (avoids yfinance network dependency).
# BN futures close ≈ BN spot within ~0.1%; sufficient for regime detection + velocity band.
spot_close = _daily_close.to_dict()   # {date → last daily close from 1m futures cache}
_spot_dates_sorted = sorted(spot_close.keys())


# ── Helpers ───────────────────────────────────────────────────────────────────
def get_walls_from_bhavcopy(df_strikes, spot, band=3000):
...
BN_LOT           = 30      # BankNifty lot size (30 units per lot)
NIFTY_STEP       = 100      # BankNifty strike interval
...
MIN_REGIME_PCT   = 1.0     # minimum |5d return| % to enter any trade.
                           # Rationale: in flat 5-day windows BankNifty's OI velocity,
                           # strike defense, and basis all accumulate directional noise.
MIN_VOL_PCT      = 0.85    # minimum 20-day realized vol (std of daily % returns) to trade.
                           # Rationale: below 0.85%, BankNifty option premiums don't move
                           # enough to generate returns even on correct directional bets.
                           # Backtest: vol < 0.85% → 9 trades, 33% WR, -635.8 pts.
                           #           vol ≥ 0.85% → 9 trades, 89% WR, +1671.7 pts.
                           # Threshold gap: lowest passing trade = Feb 13 (1.005%),
                           #               highest blocked trade = Feb 2 (0.846%) → 16bp gap.
                           # Economic logic: low-vol BankNifty = option theta eats directional
                           # gains; IV compression kills premium on correct bets.
...
TRAIL_ACTIVATE_PCT = 0.40    # trailing stop activates once option is up 40%
TRAIL_LOCK_PCT     = 0.15    # once activated, lock in ≥15% profit (exit if drops below)
# Filter thresholds:
# F0:   |5d return| < MIN_REGIME_PCT → skip day (flat market, OI signals unreliable)
# F_VOL: 20d realized vol < MIN_VOL_PCT → skip day (premiums won't move enough to profit)
# F1:   extreme_regime = |5d return| > 3% → require |score| >= 0.50
# F2:   PCR bearish (-1) + LONG + score < 0.55 → suppress
# F3:   clf disagrees with engine direction → hard block (LONG+FII_BEAR or SHORT+FII_BULL)
# F4a:  OI quadrant = -1 (bearish OI) + LONG → suppress
...
# TRAILING STOP: once option up ≥40%, lock in ≥15% profit
# NOTE: F6 (moderate FII cash + 10d return gate) was REMOVED — curve-fit to Jan 2026.
...
for i, td in enumerate(trade_dates):
    ...
    # ── F0: Flat-regime skip ──────────────────────────────────────────────────
    # In sideways 5-day windows OI patterns are noisy — skip this day entirely.
    if abs(regime_5d_return) < MIN_REGIME_PCT:
        log.debug("F0_flat_regime skip date=%s regime_5d_return=%.2f%% < %.1f%%",
                  td, regime_5d_return, MIN_REGIME_PCT)
        continue   # skip to next trading day

    # ── F_VOL: Low realized-vol skip ─────────────────────────────────────────
    # In low-volatility environments BankNifty option premiums don't move enough
    # to generate returns even on correct directional bets.
    # 20-day realized vol < 0.85%: options are thin, theta eats gains.
    ...
    # Pre-build CLF lookup dicts for this day.
    # IMPORTANT: each classifier needs its own lookup dict because _push_clf_bar
    # tracks pushed hhmm values in lk['pushed'] — a shared dict would cause clf 2
    # and clf 3 to see every bar as "already pushed" and silently skip.
    clf_lookups    = None   # Nifty reference
    clf_lookups_bn = None   # BankNifty-calibrated (primary)
    clf_lookups_cmb= None   # Combined
    fii_dii_live   = None   # primary signal: BN-calibrated classifier result
    fii_dii_nifty  = None   # reference: Nifty-thresholds result
    fii_dii_comb   = None   # combined-thresholds result
    if _FII_CLF_AVAILABLE and str(td) in opt_oi_cache:
        try:
            _day_oi_ref = opt_oi_cache[str(td)]
            if _clf is not None:
                clf_lookups     = _build_clf_lookups(_day_oi_ref, day_df, spot_at_0945)
            if _clf_bn is not None:
                clf_lookups_bn  = _build_clf_lookups(_day_oi_ref, day_df, spot_at_0945)
            if _clf_combined is not None:
                clf_lookups_cmb = _build_clf_lookups(_day_oi_ref, day_df, spot_at_0945)
            for _c in (_clf, _clf_bn, _clf_combined):
                if _c is not None:
                    _c._buffer.clear()
                    _c._daily_oi_adds.clear()
                    _c._prev_snap = None
        except Exception as _clf_build_err:
            log.warning("CLF lookup build failed date=%s error=%s", td, _clf_build_err)
            clf_lookups = clf_lookups_bn = clf_lookups_cmb = None
    ...
    _any_clf_lookups = clf_lookups or clf_lookups_bn or clf_lookups_cmb
    _skip_loop = (not _has_oi_data) and (not _any_clf_lookups)
    if _skip_loop:
    ...
    final_fii        = None
    max_pnl_pct      = 0.0     # running high-water mark of pnl_pct during position
    trail_activated  = False    # True once max_pnl_pct >= TRAIL_ACTIVATE_PCT
    ...
    for bar_idx in (range(len(day_df)) if not _skip_loop else []):
        ...
        # Each classifier has its own lookup dict (separate pushed sets) to avoid
        # the first push marking the bar as done and silently skipping clf 2 and 3.
        try:
            if clf_lookups is not None and _clf is not None:
                if _push_clf_bar(_clf, clf_lookups, current_hhmm, spot_at_0945):
                    _r = _clf.classify(fii_cash_5d_norm=fii_cash_5d_norm)
                    if not (_r.get('attribution') == 'UNKNOWN' and _r.get('confidence', 0) == 0):
                        fii_dii_nifty = _r

            if clf_lookups_bn is not None and _clf_bn is not None:
                if _push_clf_bar(_clf_bn, clf_lookups_bn, current_hhmm, spot_at_0945):
                    _r = _clf_bn.classify(fii_cash_5d_norm=fii_cash_5d_norm)
                    if not (_r.get('attribution') == 'UNKNOWN' and _r.get('confidence', 0) == 0):
                        fii_dii_live = _r   # primary: BN thresholds

            if clf_lookups_cmb is not None and _clf_combined is not None:
                if _push_clf_bar(_clf_combined, clf_lookups_cmb, current_hhmm, spot_at_0945):
                    _r = _clf_combined.classify(fii_cash_5d_norm=fii_cash_5d_norm)
                    if not (_r.get('attribution') == 'UNKNOWN' and _r.get('confidence', 0) == 0):
                        fii_dii_comb = _r
        except Exception:
            pass
        ...
        # ── Hot path: in-position SL/TP/TRAIL/EOD check every bar (no signal needed) ─
        if in_position:
            ...
            if opt_px_now is not None:
                ...
                # Trailing stop: once activated, exit if profit drops back below lock level
                trail_hit = trail_activated and pnl_pct <= TRAIL_LOCK_PCT
                if sl_hit or tp_hit or trail_hit or eod:
                    exit_opt_px = opt_px_now
                    exit_time   = current_hhmm
                    exit_reason = ('SL'          if sl_hit    else
                                   'TP'          if tp_hit    else
                                   'TRAIL_STOP'  if trail_hit else 'EOD')
                    in_position = False
                    break
            elif eod:
                exit_time   = current_hhmm
        ...
        # ── F2: PCR soft veto (LONG only) ────────────────────────────────────
        # Only block longs when PCR is bearish and score is marginal.
        if effective_dir == 1 and state.pcr == -1 and state.score < 0.55:
            effective_dir = 0

        # ── F3: clf_bn directional disagreement → suppress ───────────────────
        # clf_bn fires FII_BEAR on ~78% of days due to structural bear bias in
        # the Jan-Apr calibration data — it is NOT a reliable direction driver.
        # It IS a reliable quality gate: when clf disagrees with engine direction,
        # accuracy drops sharply.  Block entries where clf and engine conflict.
        #   • engine LONG  + clf FII_BEAR → suppress (institutional bears, don't fight)
        #   • engine SHORT + clf FII_BULL → suppress (institutional unwinding, don't chase)
        # Symmetric: no score threshold — hard block on directional disagreement.
        if fii_dii_live is not None:
            _clf_attr = fii_dii_live.get('attribution', '')
    ...
    if entry_opt_px is not None and exit_opt_px is not None:
        ...
        pnl_inr      = round(pnl_pts * BN_LOT, 2)
    ...
    # For BN + Combined classifiers, use last known live result (no separate at-entry tracking needed)
    use_fii_bn   = fii_dii_live   # last bar from BN classifier
    use_fii_comb = fii_dii_comb   # last bar from Combined classifier
    ...
    row.update({
        ...
        # FII/DII classifiers — Nifty thresholds (reference), BN thresholds (primary), Combined
        ...
        'clf_bn_attribution':  use_fii_bn.get('attribution', 'N/A') if use_fii_bn else 'N/A',
        'clf_bn_confidence':   round(use_fii_bn.get('confidence', 0.0), 3) if use_fii_bn else 0.0,
        'clf_bn_fii_score':    round(use_fii_bn.get('fii_score', 0.0), 3) if use_fii_bn else 0.0,
        'clf_cmb_attribution': use_fii_comb.get('attribution', 'N/A') if use_fii_comb else 'N/A',
        'clf_cmb_confidence':  round(use_fii_comb.get('confidence', 0.0), 3) if use_fii_comb else 0.0,
        'clf_cmb_fii_score':   round(use_fii_comb.get('fii_score', 0.0), 3) if use_fii_comb else 0.0,
...
_CSV_OUT_NAME = _OUT_OVERRIDE if _OUT_OVERRIDE else 'trade_log_options_banknifty.csv'
...
has_opt_price = any(
    isinstance(df, pd.DataFrame) and not df.empty and 'close' in df.columns
    for day in opt_oi_cache.values()
    for strike_sides in day.values()
    for df in strike_sides.values()
...
if not has_opt_price:
    print(
        ...
        "Fix: re-fetch option OI with OHLCV enabled (fetch_option_oi_BANKNIFTY.py).\n"
    )

if res.empty or 'direction' not in res.columns:
    print('\nNo trades fired in the given date range.')
    import sys; sys.exit(0)

print('\n=== BANKNIFTY V3 BACKTEST — REAL-TIME BAR-BY-BAR ===')
print(f"  SL={SL_PCT*100:.0f}%  TP={TP_PCT*100:.0f}%  EOD={EOD_EXIT_HHMM}  "
      f"min_bar={MIN_SIGNAL_BAR} (11:00)  last_entry={LAST_ENTRY_HHMM}  "
      f"filters=F0(regime>={MIN_REGIME_PCT}%)+F_VOL(vol>={MIN_VOL_PCT}%)+F1+F2+F3_sym+F4a+F4b+F5+MinHist({MIN_PRIOR_DAYS}d)")
...
export_cols = [
    ...
    'strike_defense', 'fii_signature', 'has_opt_oi',
    'fii_attribution', 'clf_confidence', 'clf_fii_score',
    'clf_bn_attribution', 'clf_bn_confidence', 'clf_bn_fii_score',
    'clf_cmb_attribution', 'clf_cmb_confidence', 'clf_cmb_fii_score',
    'dte', 'regime_5d_ret', 'extreme_regime', 'call_wall_in', 'put_wall_in',
    'pcr_input', 'fii_fut', 'fii_cash', 'day_open', 'notes',
]
